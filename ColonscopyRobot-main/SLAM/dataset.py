...
class ColonDataset():
    '''
    path example: 'path/to/simulator/case1'
    '''
    def __init__(self, path):
        Cam = namedtuple('cam', 'fx fy cx cy width height baseline')
        self.cam = Cam(90, 90, 160.0, 180.0, 480, 640, 1)


        path = os.path.expanduser(path)
        
        timestamps = np.loadtxt(os.path.join(path, 'LMB_Camera Position L Data.txt'), delimiter=", ", usecols=(0, 1, 2), unpack = True)
